...
public class LineCtrl : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        edgeCollider.isTrigger = true;
         LevelManagerBeeGame.Ins.isDrawed = false;
    }
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            SoundFXMNG.Ins.PlaySFX(SoundFXMNG.Ins.bee);
             LevelManagerBeeGame.Ins.isDrawed = true;
            edgeCollider.SetPoints(listPoint);
             LevelManagerBeeGame.Ins.StartGame();
            UIManagerBeeGame.Ins.InGameCanvas.ActiveClock();   
            rb.simulated = true;
            this.enabled = false;
        ...
        if (UIManagerBeeGame.Ins.InGameCanvas.fillBar.fillAmount <= 0)
            return;
        ...
        for (int i = 0; i < listPoint.Count; i++)
        {
            ...
            UIManagerBeeGame.Ins.InGameCanvas.DecreaseFillAmount();
        }
    }
}
