...
public class ShopCanvas : UICanvas
{
    ...
    private void OnShopItemBtnClicked(int itemIndex)
    {
        ...
        // Purchase logic
        if (LevelManagerBeeGame.Ins.money >= 500 && !listShopIT[itemIndex].isBought)
        {
            LevelManagerBeeGame.Ins.money -= 500;
            LevelManagerBeeGame.Ins.LoadMoney(moneyTxt);
        ...
        else if (LevelManagerBeeGame.Ins.money < 500 && !listShopIT[itemIndex].isBought)
        {
    ...
    {
        LevelManagerBeeGame.Ins.LoadIDForPlayer(count);
    }
}
